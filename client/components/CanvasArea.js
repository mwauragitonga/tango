...
const CanvasArea = forwardRef(
  (
    ...
    useImperativeHandle(ref, () => ({
      ...
      addVideo: (...args) => addVideo(...args),
      addAudio: (...args) => addAudio(...args),
      updateZoom: (...args) => updateZoom(...args),
    ...
    const addImage = (src, x, y, width, height, onLoaded) => {
      if (!src) return;
    ...
    const addVideo = (src, x, y, width, height, onLoaded) => {
      if (!src) return;
      const stage = stageRef.current;
      if (!stage) {
